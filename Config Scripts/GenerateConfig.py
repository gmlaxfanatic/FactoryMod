...
gpMod=1*gMod#Global Production Modifier
guMod=1*gMod#Global Upgrade Modifier
def main():
    print 'Running....'
    createGraphs()
    createConfigFile()
...
def createArmor():
    ...
    mMod=0.1#Percent of the original cost required for maintenance
    ...
    for i in reversed(range(len(techs))):
        ...
        for item in items:
            ...
            recipes['E'+tech+item]=Recipe(identifier='E'+tech+item,name='Forge '+tech+' '+item+'s Efficiently',inputs=inputs1,outputs=outputs1,maintenance=maintenance)
            #Upgrade to Efficient Production
            inputs2=[ItemStack(name='Coal',amount=aMod*eMod*iCosts[item]*uCosts[tech])]
            outputRecipes2=[recipes['E'+tech+item]]
            recipes['UE'+tech+item]=Recipe(identifier='UE'+tech+item,name='Upgrade to Efficent '+tech+' '+item+' Production',inputs=inputs2,outputRecipes=outputRecipes2)
            #Upgrade to Standard Production
            inputs3=[ItemStack(name='Coal',amount=aMod*iCosts[item]*uCosts[tech])]
...
def createTools():
    ...
    tMod=16#modifier for tool recipes
    pMod=2#Modifier for production recipes
    eMod=5#Modified for efficient upgrade
    eff=.5#Decrease in cost of efficient plant
    bulk=10#Bulk amount produced by efficient plant
    mMod=0.3#Percent of the original cost required for maintenance
    ...
    for tech in reversed(techs):
        for tool in tools:
            #Efficient Production Recipe
            inputs=[ItemStack(name=resources[tech],amount=gMod*tMod*pMod*iCosts[tool]*bulk*eff*pCosts[tech])]
            outputs=[ItemStack(material=tech.upper()+'_'+tool.upper(),name=tech+' '+tool,amount=bulk,durability=-218*(tech=='Gold'))]
            maintenance=mMod*gMod*tMod*eMod*iCosts[tool]*uCosts[tech]
            ...
            inputs=[ItemStack(name='Coal',amount=gMod*tMod*eMod*iCosts[tool]*uCosts[tech])]
            ...
            inputs=[ItemStack(name=resources[tech],amount=gMod*tMod*pMod*iCosts[tool]*pCosts[tech])]
            ...
            inputs=[ItemStack(name='Coal',amount=gMod*tMod*iCosts[tool]*uCosts[tech])]
            outputRecipes=[recipes[tech+tool],recipes['UE'+tech+tool]]
            if tech!='Diamond':
                outputRecipes+=[recipes['U'+techs[techs.index(tech)+1]+tool]]
            recipes['U'+tech+tool]=Recipe('U'+tech+tool,name='Upgrade to '+tech+' '+tool+' Production',inputs=inputs,outputRecipes=outputRecipes)
    for tool in tools:
...
def createMetalTech():
    ...
    techInputs={'Stone':('Cobble','COBBLE'),'Iron':('Iron Ingot','IRON_INGOT'),'Gold':('Gold Ingot','GOLD_INGOT'),'Diamond':('Diamond','DIAMOND'),'Emerald':('Emerald','EMERALD')}
    ...
    for i in reversed(range(len(techLevels))):
        tech=techLevels[i]
        #Efficient Recipe
        inputs=[ItemStack(name=techInputs[tech][0],material=techInputs[tech][1],amount=meMod*iMod[tech]*bulk*eff)]
        ...
        recipes['E'+tech]=Recipe(identifier='E'+tech,name='Process '+tech+' Efficiently',inputs=inputs,outputs=outputs,maintenance=maintenance)
        #Standard Recipe
        inputs=[ItemStack(name=techInputs[tech][0],material=techInputs[tech][1],amount=meMod*iMod[tech])]
        outputs=[ItemStack(name=techOutputs[tech],amount=meMod*oMod[tech])]
        recipes[tech]=Recipe(identifier=tech,name='Process '+tech,inputs=inputs,outputs=outputs)
        #Upgrade to efficient Recipe
        inputs=[ItemStack(name=techOutputs[tech],amount=meMod*uCost[tech]*eMod)]
        outputRecipes=[recipes['E'+tech]]
        recipes['UE'+tech]=Recipe(identifier='UE'+tech,name='Upgrade to Efficient '+tech+' Processing',inputs=inputs,outputRecipes=outputRecipes)
        #Upgrade to standard recipe
        inputs=[ItemStack(name=techInputs[tech][0],material=techInputs[tech][1],amount=meMod*uCost[tech])]
        outputRecipes=[recipes[tech],recipes['UE'+tech]]
        if i!=len(techLevels)-1:
            outputRecipes.append(recipes['U'+techLevels[i+1]])
        recipes['U'+tech]=Recipe(identifier='U'+tech,name='Upgrade to '+tech+' Processing',inputs=inputs,outputRecipes=outputRecipes)
    factories['Smelter']=Factory(identifier='Smelter',name='Smelter',inputs=[ItemStack(name='Cobble',material='COBBLE',amount=64)],maintenanceInputs=[ItemStack(name='Diamond',material='DIAMOND')])
...
def createFood():
    ...
    inputs['Golden_Carrot']=[ItemStack(material='CARROT',amount=pMod),ItemStack(material='GOLD_BLOCK',amount=3*pMod)]
    ...
    inputs['Baked_Potato']=[ItemStack(material='POTATO',amount=pMod)]
    ...
    inputs['Cookie']=[ItemStack(name='Cocoa',material='COCOA',amount=pMod),ItemStack(name='Wheat',material='WHEAT',amount=pMod)]
    ...
    outputs['Mushroom_Soup']=[ItemStack(material='MUSHROOM_STEW',amount=3*fMod)]
    ...
    recipes['U_E_Beef']=Recipe(identifier='U_E_Beef',name='Upgrade to Efficient Beef',inputs=inputs['U_E_Beef'],outputRecipes=[recipes['E_Beef']])
    inputs['U_E_Golden_Carrot']=[ItemStack(material='ENDER_PEARL',amount=uMod),ItemStack(material='GHAST_TEAR',amount=uMod)]
    recipes['U_E_Golden_Carrot']=Recipe(identifier='U_E_Golden_Carrot',name='Upgrade to Efficient Golden Carrots',inputs=inputs['U_E_Golden_Carrot'],outputRecipes=[recipes['E_Golden_Carrot']])
    inputs['U_Golden_Carrot_and_Beef']=[ItemStack(material='ENDER_PEARL',amount=uMod),ItemStack(material='GHAST_TEAR',amount=uMod)]
    recipes['U_Golden_Carrot_and_Beef']=Recipe(identifier='U_Golden_Carrot_and_Beef',name='Upgrade to Golden Carrots',inputs=inputs['U_Golden_Carrot_and_Beef'],outputRecipes=[recipes['Golden_Carrot'],recipes['U_E_Golden_Carrot'],recipes['Beef'],recipes['U_E_Beef']])
    inputs['U_E_Golden_Apple']=[ItemStack(material='ENDER_PEARL',amount=uMod),ItemStack(material='GHAST_TEAR',amount=uMod)]
    recipes['U_E_Golden_Apple']=Recipe(identifier='U_E_Golden_Apple',name='Upgrade to Efficient Golden Apples',inputs=inputs['U_E_Golden_Apple'],outputRecipes=[recipes['E_Golden_Apple']])
    inputs['U_Golden_Apple']=[ItemStack(material='ENDER_PEARL',amount=uMod),ItemStack(material='GHAST_TEAR',amount=uMod)]
    recipes['U_Golden_Apple']=Recipe(identifier='U_Golden_Apple',name='Upgrade to Golden Apples',inputs=inputs['U_Golden_Apple'],outputRecipes=[recipes['Golden_Apple'],recipes['U_E_Golden_Apple']])
    inputs['U_E_Cake']=[ItemStack(material='SPIDER_EYE',amount=uMod),ItemStack(name='Gunpowder',material='SULPHUR',amount=uMod)]
    recipes['U_E_Cake']=Recipe(identifier='U_E_Cake',name='Upgrade to Efficient Cake',inputs=inputs['U_E_Cake'],outputRecipes=[recipes['E_Cake']])
    inputs['U_E_Chicken']=[ItemStack(material='SPIDER_EYE',amount=uMod),ItemStack(name='Gunpowder',material='SULPHUR',amount=uMod)]
    recipes['U_E_Chicken']=Recipe(identifier='U_E_Chicken',name='Upgrade to Efficient Chicken',inputs=inputs['U_E_Chicken'],outputRecipes=[recipes['E_Chicken']])
    ...
    recipes['U_Cake_and_Chicken']=Recipe(identifier='U_Cake_and_Chicken',name='Upgrade to Cake and Chicken',inputs=inputs['U_Cake_and_Chicken'],outputRecipes=[recipes['Cake'],recipes['U_E_Cake'],recipes['Chicken'],recipes['U_E_Chicken'],recipes['U_Golden_Apple']])
    inputs['U_E_Pumpkin_Pie']=[ItemStack(material='SPIDER_EYE',amount=uMod),ItemStack(name='Gunpowder',material='SULPHUR',amount=uMod)]
    recipes['U_E_Pumpkin_Pie']=Recipe(identifier='U_E_Pumpkin_Pie',name='Upgrade to Efficient Pumpkin Pie',inputs=inputs['U_E_Pumpkin_Pie'],outputRecipes=[recipes['E_Pumpkin_Pie']])
    inputs['U_E_Pork']=[ItemStack(material='SPIDER_EYE',amount=uMod),ItemStack(name='Gunpowder',material='SULPHUR',amount=uMod)]
    recipes['U_E_Pork']=Recipe(identifier='U_E_Pork',name='Upgrade to Efficient Pork',inputs=inputs['U_E_Pork'],outputRecipes=[recipes['E_Pork']])
    ...
    recipes['U_Pumpkin_Pie_and_Pork']=Recipe(identifier='U_Pumpkin_Pie_and_Pork',name='Upgrade to Pumpkin Pie',inputs=inputs['U_Pumpkin_Pie_and_Pork'],outputRecipes=[recipes['Pumpkin_Pie'],recipes['U_E_Pumpkin_Pie'],recipes['Pork'],recipes['U_E_Pork'],recipes['U_Golden_Carrot_and_Beef']])
    inputs['U_E_Bread']=[ItemStack(material='BONE',amount=uMod),ItemStack(material='ROTTEN_FLESH',amount=uMod)]
    recipes['U_E_Bread']=Recipe(identifier='U_E_Bread',name='Upgrade to Efficient Bread',inputs=inputs['U_E_Bread'],outputRecipes=[recipes['E_Bread']])
    inputs['U_E_Baked_Potato']=[ItemStack(material='BONE',amount=uMod),ItemStack(material='ROTTEN_FLESH',amount=uMod)]
    recipes['U_E_Baked_Potato']=Recipe(identifier='U_E_Baked_Potato',name='Upgrade to Efficient Baked Potato',inputs=inputs['U_E_Baked_Potato'],outputRecipes=[recipes['E_Baked_Potato']])
    inputs['U_E_Cookie']=[ItemStack(material='BONE',amount=uMod),ItemStack(material='ROTTEN_FLESH',amount=uMod)]
    recipes['U_E_Cookie']=Recipe(identifier='U_E_Cookie',name='Upgrade to Efficient Cookie',inputs=inputs['U_E_Cookie'],outputRecipes=[recipes['E_Cookie']])
    inputs['U_E_Mushroom_Soup']=[ItemStack(material='BONE',amount=uMod),ItemStack(material='ROTTEN_FLESH',amount=uMod)]
    recipes['U_E_Mushroom_Soup']=Recipe(identifier='U_E_Mushroom_Soup',name='Upgrade to Efficient Mushroom Soup',inputs=inputs['U_E_Mushroom_Soup'],outputRecipes=[recipes['E_Mushroom_Soup']])
...
def createEnchanting():
    ...
    for branch in enchantments.keys():#Go through different classes of enchanments
        for tier in reversed(range(tiers)):#Goes thougheach tier of more advanced enchantments
            ...
            recipes['U'+branch+str(tier)]=Recipe(identifier='U'+branch+str(tier),name='Upgrade to Level '+str(tier+1)+' '+branch+' Enchantments',inputs=inputs,outputRecipes=outputRecipes)
...
def createCauldron():
    ...
    outputRecipes={}
    techs=['Clear Potion','Diffuse Potion','Artless Potion','Thin Potion','Bungling Potion']
    tMod={'Clear Potion':1,'Diffuse Potion':1,'Artless Potion':1,'Thin Potion':1,'Bungling Potion':1}
    cMod=1*gMod
    uMod=10
    euMod=5
    eff=.7
    ...
    defined_inputs[techs[0]][0]=[ItemStack(name='Wheat',material='WHEAT',amount=cMod*tMod[techs[0]]*32),ItemStack(name='Cocoa',material='COCOA',amount=cMod*tMod[techs[0]]*32)]
    defined_inputs[techs[0]][1]=[ItemStack(name='Potato',material='POTATO',amount=cMod*tMod[techs[0]]*32),ItemStack(name='Brown Mushroom',material='BROWN_MUSHROOM',amount=cMod*tMod[techs[0]]*8)]
    ...
    for i in reversed(range(len(techs))):
        ...
        for j in [0,1]:
            ...
            recipes[identifier['U_E_Potion']]=Recipe(identifier=identifier['U_E_Potion'],name='Upgrade to Producing '+tech+'s Efficiently',inputs=inputs[identifier['U_E_Potion']],outputRecipes=outputRecipes[identifier['U_E_Potion']])
            ...
            if i!=0:
                recipes[identifier['U_Potion']]=Recipe(identifier=identifier['U_Potion'],name='Upgrade to '+tech+'s',inputs=inputs[identifier['U_Potion']],outputRecipes=outputRecipes[identifier['U_Potion']])
